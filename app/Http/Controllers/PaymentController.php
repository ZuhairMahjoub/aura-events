...
class PaymentController extends Controller
{
    ...
    public function uploadProof(
        ...
        // 👑 التعديل هنا: البحث عن الحالات pending و confirmed بدلاً من completed
        $existingPayment = Payment::where('booking_id', $request->booking_id)
            ->whereIn('status', ['pending', 'confirmed'])
            ->first();

        if ($existingPayment) {
            // 👑 التعديل هنا: فحص حالة confirmed
            $message = $existingPayment->status === 'confirmed'
    ...
    public function confirmPayment($paymentId)
    {
        ...
        // 👑 التعديل هنا: التحقق من حالة confirmed
        ...
        \Illuminate\Support\Facades\DB::transaction(function () use ($payment) {
            // 👑 التعديل هنا: تحديث حالة الدفع إلى confirmed
            $payment->update(['status' => 'confirmed']);

            // 👑 التعديل هنا: تحديث حالة الحجز إلى confirmed
    ...
    public function rejectPayment(Request $request, $paymentId)
    {
        ...
        // هنا الحالة failed متطابقة مع المايجريشن تبعك تماماً
        $payment->update([
            'status' => 'failed',
