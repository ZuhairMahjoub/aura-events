...
class StoreListingRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $rules = [
            ...
            // ✨ إضافة حقول الأمان والإلغاء لكي يمررها الـ validated() بنجاح
            ...
            'variants.*.id'              => ['nullable', 'string'], // مهم جداً لحالة الـ Update
        ...
        if ($isPhysicalProduct) {
            ...
            // --- نظام: availabilities (تم دمج فحص التكرار لمنع الـ Overwrite) ---
            ...
            $rules['variants.*.availabilities.*.id']                 = ['nullable', 'string']; // مهم للـ Update لتجنب تكرار التواريخ
            ...
            $rules['variants.*.date_range.slots.*.start_time']         = ['required_with:variants.*.date_range.slots', 'date_format:H:i'];
            $rules['variants.*.date_range.slots.*.end_time']           = ['required_with:variants.*.date_range.slots', 'date_format:H:i'];
            $rules['variants.*.date_range.slots.*.slot_name']          = ['nullable'];           // ✅ أضف هذا
            $rules['variants.*.date_range.slots.*.remaining_capacity'] = ['nullable', 'integer', 'min:1']; // ✅ وهذا أيضاً
        }
